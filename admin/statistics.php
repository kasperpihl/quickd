...
function cmp($a, $b)
{
    $aInvited = (isset($a->value->invited) && $a->value->invited==true && $a->value->invited!='false');
    $bInvited = (isset($b->value->invited) && $b->value->invited==true && $b->value->invited!='false');
...
if(!empty($users)){
	echo '<table><thead><th>Email</th><th>Facebook</th><th>Oprettet</th><th>Inviteret</th></thead><tbody>';
	foreach($users as $user){
		$fb = (isset($user->value->fb_info)) ? 'X' :'';
		echo '<tr>';
		echo '<td>'.$user->value->email . '</td><td>'.$fb.'</td><td>'.date('j/n H:i',$user->key).'</td>';
		echo '<td>'.((isset($user->value->invited) && $user->value->invited==true && $user->value->invited!='false') ? 'Ja' : 'Nej').'</td>';
		echo '</tr>';
	}
	echo '</tbody></table>';
}
